...
function fmtTime(dateStr: string | null): string {
  ...
  try {
    if (dateStr.includes('T')) {
      ...
      if (tp) {
        const [h, m] = tp.split(':')
        const hr = Number.parseInt(h || '0', 10)
        ...
        <div class="relative px-4 lg:px-8 py-6 lg:py-8">
          <div class="flex items-start justify-between gap-4 mb-4">
            <div class="flex items-center gap-2 flex-wrap">
              ...
              <Badge v-if="schedule.item" variant="outline" class="text-[11px] font-medium bg-background/80 backdrop-blur-sm">
                {{ schedule.item }}
              </Badge>
              <Badge v-if="schedule.perDiem" variant="secondary" class="text-[10px] h-6 gap-1 font-bold">
                <Icon name="i-lucide-coins" class="size-3" />Per Diem
              </Badge>
              <Badge v-if="schedule.notifyAssignees" variant="secondary" class="text-[10px] h-6 gap-1 font-bold">
                <Icon name="i-lucide-bell" class="size-3" />Notify
              </Badge>
            </div>
            <div v-if="schedule.estimate" class="shrink-0">
              <div class="rounded-lg bg-primary/5 border border-primary/15 px-4 py-2 backdrop-blur-sm">
                <p class="text-[9px] font-bold uppercase tracking-widest text-primary/70">
                  Estimate
                </p>
                <p class="text-lg font-black text-primary tabular-nums tracking-tight">
                  {{ schedule.estimate }}
                </p>
              </div>
            </div>
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div class="rounded-xl bg-card/80 backdrop-blur-sm border shadow-sm p-4 flex items-center gap-3">
              <div class="size-11 rounded-lg bg-gradient-to-br from-emerald-500 to-emerald-600 flex items-center justify-center shadow-md shadow-emerald-500/20">
                <Icon name="i-lucide-clock" class="size-5 text-white" />
              </div>
              <div>
                <p class="text-xs font-semibold">
                  {{ fmtTime(schedule.fromDate) }} – {{ fmtTime(schedule.toDate) }}
                </p><p class="text-[10px] text-muted-foreground">
                  {{ scheduleDurationHours ? `${scheduleDurationHours} hrs` : 'Work Hours' }}
                </p>
              </div>
            </div>
            <div class="rounded-xl bg-card/80 backdrop-blur-sm border shadow-sm p-4 flex items-center gap-3">
              <div class="size-11 rounded-lg bg-gradient-to-br from-violet-500 to-violet-600 flex items-center justify-center shadow-md shadow-violet-500/20">
                <Icon name="i-lucide-users" class="size-5 text-white" />
              </div>
              <div>
                <p class="text-xs font-semibold">
                  {{ totalCrewSize }} members
                </p><p class="text-[10px] text-muted-foreground">
                  Crew Size
                </p>
              </div>
            ...
            <div class="rounded-xl bg-card/80 backdrop-blur-sm border shadow-sm p-4 flex items-center gap-3">
              <div class="size-11 rounded-lg bg-gradient-to-br from-amber-500 to-orange-500 flex items-center justify-center shadow-md shadow-amber-500/20">
                <Icon name="i-lucide-timer" class="size-5 text-white" />
              </div>
              <div>
                <p class="text-xs font-semibold">
                  {{ fmtNum(totalTimeCardHours) }} hrs
                </p><p class="text-[10px] text-muted-foreground">
                  Time Logged
                </p>
              </div>
      ...
      <div class="px-4 lg:px-8 border-b">
        <div class="flex gap-0.5 -mb-px overflow-x-auto">
          ...
          >
            ...
            <Badge v-if="tab.count !== null" variant="secondary" class="h-4 min-w-4 justify-center px-1 text-[9px] font-bold tabular-nums" :class="activeTab === tab.id ? 'bg-primary/15 text-primary' : ''">
              {{ tab.count }}
            </Badge>
          </button>
        </div>
      ...
      <div class="px-4 lg:px-8 py-6">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-6">
              ...
              <Card>
                <CardHeader class="pb-3">
                  <CardTitle class="text-sm font-bold flex items-center gap-2">
                    <Icon name="i-lucide-shield-check" class="size-4 text-primary" />Leadership
                  </CardTitle>
                </CardHeader>
                <CardContent class="space-y-4">
                  <div v-if="schedule.projectManagerName" class="flex items-center gap-4 p-3 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors">
                    <Avatar class="size-12 border-2 border-primary/20">
                      <AvatarImage v-if="schedule.projectManagerAvatar" :src="schedule.projectManagerAvatar" /><AvatarFallback class="text-sm font-bold bg-primary/10 text-primary">
                        {{ getInitials(schedule.projectManagerName) }}
                      </AvatarFallback>
                    </Avatar>
                    <div class="flex-1 min-w-0">
                      <p class="text-sm font-bold">
                        {{ schedule.projectManagerName }}
                      </p><p class="text-xs text-muted-foreground">
                        Project Manager
                      </p>
                    </div>
                    <Badge variant="outline" class="text-[10px] font-bold bg-primary/5 text-primary border-primary/20 shrink-0">
                      PM
                    </Badge>
                  </div>
                  <div v-if="schedule.foremanDisplayName" class="flex items-center gap-4 p-3 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors">
                    <Avatar class="size-12 border-2 border-amber-500/20">
                      <AvatarImage v-if="schedule.foremanAvatar" :src="schedule.foremanAvatar" /><AvatarFallback class="text-sm font-bold bg-amber-500/10 text-amber-600 dark:text-amber-400">
                        {{ getInitials(schedule.foremanDisplayName) }}
                      </AvatarFallback>
                    </Avatar>
                    <div class="flex-1 min-w-0">
                      <p class="text-sm font-bold">
                        {{ schedule.foremanDisplayName }}
                      </p><p class="text-xs text-muted-foreground">
                        Foreman
                      </p>
                    </div>
                    <Badge variant="outline" class="text-[10px] font-bold bg-amber-500/5 text-amber-600 border-amber-500/20 dark:text-amber-400 shrink-0">
                      Foreman
                    </Badge>
                  </div>
                  <div v-if="!schedule.projectManagerName && !schedule.foremanDisplayName" class="text-xs text-muted-foreground italic py-4 text-center">
              ...
              <Card v-if="schedule.description">
                <CardHeader class="pb-3">
                  <CardTitle class="text-sm font-bold flex items-center gap-2">
                    <Icon name="i-lucide-file-text" class="size-4 text-primary" />Job Description
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <p class="text-sm text-foreground/80 leading-relaxed whitespace-pre-wrap">
              ...
              <Card v-if="timeCards.length > 0">
                <CardHeader class="pb-3">
                  <CardTitle class="text-sm font-bold flex items-center gap-2">
                    <Icon name="i-lucide-bar-chart-3" class="size-4 text-primary" />Hours Breakdown
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div class="grid grid-cols-3 gap-4">
                    <div class="rounded-lg bg-gradient-to-br from-emerald-500/10 to-emerald-500/5 p-4 ring-1 ring-emerald-500/15">
                      <div class="flex items-center gap-2 mb-2">
                        <div class="size-7 rounded-md bg-emerald-500/15 flex items-center justify-center">
                          <Icon name="i-lucide-map-pin" class="size-3.5 text-emerald-500" />
                        </div><p class="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                          Site
                        </p>
                      </div>
                      <p class="text-xl font-black tabular-nums text-emerald-600 dark:text-emerald-400">
                        {{ fmtNum(totalSiteHours) }}
                      </p><p class="text-[10px] text-muted-foreground">
                        hours
                      </p>
                    </div>
                    <div class="rounded-lg bg-gradient-to-br from-blue-500/10 to-blue-500/5 p-4 ring-1 ring-blue-500/15">
                      <div class="flex items-center gap-2 mb-2">
                        <div class="size-7 rounded-md bg-blue-500/15 flex items-center justify-center">
                          <Icon name="i-lucide-car" class="size-3.5 text-blue-500" />
                        </div><p class="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                          Drive
                        </p>
                      </div>
                      <p class="text-xl font-black tabular-nums text-blue-600 dark:text-blue-400">
                        {{ fmtNum(totalDriveHours) }}
                      </p><p class="text-[10px] text-muted-foreground">
                        hours
                      </p>
                    </div>
                    <div class="rounded-lg bg-gradient-to-br from-amber-500/10 to-amber-500/5 p-4 ring-1 ring-amber-500/15">
                      <div class="flex items-center gap-2 mb-2">
                        <div class="size-7 rounded-md bg-amber-500/15 flex items-center justify-center">
                          <Icon name="i-lucide-route" class="size-3.5 text-amber-500" />
                        </div><p class="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                          Distance
                        </p>
                      </div>
                      <p class="text-xl font-black tabular-nums text-amber-600 dark:text-amber-400">
                        {{ fmtNum(totalDistance, 1) }}
                      </p><p class="text-[10px] text-muted-foreground">
                        miles
                      </p>
                    </div>
            ...
            <div class="space-y-6">
              <Card>
                <CardHeader class="pb-3">
                  <CardTitle class="text-sm font-bold flex items-center gap-2">
                    <Icon name="i-lucide-info" class="size-4 text-primary" />Schedule Info
                  </CardTitle>
                </CardHeader>
                <CardContent class="space-y-3">
                  <div v-if="schedule.status" class="flex items-center justify-between">
                    <span class="text-xs text-muted-foreground">Status</span><Badge variant="outline" class="text-[10px] font-bold">
                      {{ schedule.status }}
                    </Badge>
                  </div>
                  <Separator /><div class="flex items-center justify-between">
                    <span class="text-xs text-muted-foreground">Date</span><span class="text-xs font-semibold">{{ fmtLongDate(schedule.fromDate) }}</span>
                  </div>
                  <Separator /><div class="flex items-center justify-between">
                    <span class="text-xs text-muted-foreground">Start</span><span class="text-xs font-semibold">{{ fmtTime(schedule.fromDate) }}</span>
                  </div>
                  <Separator /><div class="flex items-center justify-between">
                    <span class="text-xs text-muted-foreground">End</span><span class="text-xs font-semibold">{{ fmtTime(schedule.toDate) }}</span>
                  </div>
                  <template v-if="scheduleDurationHours">
                    <Separator /><div class="flex items-center justify-between">
                      <span class="text-xs text-muted-foreground">Duration</span><span class="text-xs font-semibold tabular-nums">{{ scheduleDurationHours }} hrs</span>
                    </div>
                  </template>
                  <template v-if="schedule.estimate">
                    <Separator /><div class="flex items-center justify-between">
                      <span class="text-xs text-muted-foreground">Estimate</span><span class="text-xs font-bold text-primary tabular-nums">{{ schedule.estimate }}</span>
                    </div>
                  </template>
                </CardContent>
              </Card>
              <Card>
                <CardHeader class="pb-3">
                  <CardTitle class="text-sm font-bold flex items-center gap-2">
                    <Icon name="i-lucide-flag" class="size-4 text-primary" />Flags
                  </CardTitle>
                </CardHeader>
                <CardContent class="space-y-3">
                  <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                      <Icon name="i-lucide-coins" class="size-4 text-amber-500" /><span class="text-xs">Per Diem</span>
                    </div><Badge :variant="schedule.perDiem ? 'default' : 'secondary'" class="text-[10px] font-bold">
                      {{ schedule.perDiem ? 'Yes' : 'No' }}
                    </Badge>
                  </div>
                  <Separator /><div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                      <Icon name="i-lucide-bell" class="size-4 text-blue-500" /><span class="text-xs">Notify</span>
                    </div><Badge :variant="schedule.notifyAssignees ? 'default' : 'secondary'" class="text-[10px] font-bold">
                      {{ schedule.notifyAssignees ? 'Yes' : 'No' }}
                    </Badge>
                  </div>
                </CardContent>
              </Card>
        ...
        <div v-else-if="activeTab === 'crew'" class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div v-if="schedule.projectManagerName" class="group rounded-xl border bg-card/80 overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5">
              ...
              <div class="p-5 flex items-center gap-4">
                <Avatar class="size-14 border-2 border-primary/20 ring-4 ring-primary/10">
                  <AvatarImage v-if="schedule.projectManagerAvatar" :src="schedule.projectManagerAvatar" /><AvatarFallback class="text-lg font-bold bg-primary/10 text-primary">
                    {{ getInitials(schedule.projectManagerName) }}
                  </AvatarFallback>
                </Avatar>
                <div class="flex-1 min-w-0">
                  <p class="text-base font-bold group-hover:text-primary transition-colors">
                    {{ schedule.projectManagerName }}
                  </p><p class="text-xs text-muted-foreground flex items-center gap-1 mt-0.5">
                    <Icon name="i-lucide-shield-check" class="size-3 text-primary" />Project Manager
                  </p>
                </div>
              </div>
            </div>
            <div v-if="schedule.foremanDisplayName" class="group rounded-xl border bg-card/80 overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5">
              ...
              <div class="p-5 flex items-center gap-4">
                <Avatar class="size-14 border-2 border-amber-500/20 ring-4 ring-amber-500/10">
                  <AvatarImage v-if="schedule.foremanAvatar" :src="schedule.foremanAvatar" /><AvatarFallback class="text-lg font-bold bg-amber-500/10 text-amber-600 dark:text-amber-400">
                    {{ getInitials(schedule.foremanDisplayName) }}
                  </AvatarFallback>
                </Avatar>
                <div class="flex-1 min-w-0">
                  <p class="text-base font-bold group-hover:text-amber-500 transition-colors">
                    {{ schedule.foremanDisplayName }}
                  </p><p class="text-xs text-muted-foreground flex items-center gap-1 mt-0.5">
                    <Icon name="i-lucide-hard-hat" class="size-3 text-amber-500" />Foreman
                  </p>
                </div>
              </div>
            </div>
          ...
          <Card v-if="schedule.assigneeDetails?.length > 0">
            <CardHeader class="pb-3">
              <CardTitle class="text-sm font-bold flex items-center gap-2">
                <Icon name="i-lucide-users" class="size-4 text-primary" />Crew Members<Badge variant="secondary" class="text-[10px] font-bold ml-auto">
                  {{ schedule.assigneeDetails.length }}
                </Badge>
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                <div v-for="(member, idx) in schedule.assigneeDetails" :key="member.id || idx" class="flex items-center gap-3 p-3 rounded-lg bg-muted/30 hover:bg-muted/50 transition-all duration-200 hover:shadow-sm group cursor-default">
                  <Avatar class="size-10 border group-hover:border-primary/30 transition-colors">
                    <AvatarImage v-if="member.avatar" :src="member.avatar" /><AvatarFallback class="text-xs font-bold text-white" :class="getAvatarColor(member.name)">
                  ...
                  <div class="size-6 rounded-full bg-muted flex items-center justify-center shrink-0 text-[10px] font-bold text-muted-foreground">
                    {{ Number(idx) + 1 }}
                  </div>
                </div>
              </div>
          ...
          <div v-if="!schedule.assigneeDetails?.length" class="flex flex-col items-center justify-center py-16">
            <div class="size-14 rounded-full bg-muted flex items-center justify-center mb-3">
              <Icon name="i-lucide-users" class="size-7 text-muted-foreground/50" />
            </div><h3 class="text-sm font-bold">
              No Crew Assigned
            </h3>
          </div>
        </div>
        ...
        <div v-else-if="activeTab === 'timecards'" class="space-y-4">
          <template v-if="timeCards.length > 0">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
              <div class="rounded-lg bg-muted/30 p-3 ring-1 ring-border/30">
                <p class="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                  Total
                </p><p class="text-lg font-black tabular-nums">
                  {{ timeCards.length }}
                </p>
              </div>
              <div class="rounded-lg bg-emerald-500/5 p-3 ring-1 ring-emerald-500/15">
                <p class="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                  Site Hours
                </p><p class="text-lg font-black tabular-nums text-emerald-600 dark:text-emerald-400">
                  {{ fmtNum(totalSiteHours) }}
                </p>
              </div>
              <div class="rounded-lg bg-blue-500/5 p-3 ring-1 ring-blue-500/15">
                <p class="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                  Drive Hours
                </p><p class="text-lg font-black tabular-nums text-blue-600 dark:text-blue-400">
                  {{ fmtNum(totalDriveHours) }}
                </p>
              </div>
              <div class="rounded-lg bg-amber-500/5 p-3 ring-1 ring-amber-500/15">
                <p class="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                  Distance
                </p><p class="text-lg font-black tabular-nums text-amber-600 dark:text-amber-400">
              ...
              <Table>
                ...
                <TableBody>
                  <TableRow v-for="(tc, idx) in timeCards" :key="tc._id || idx" class="hover:bg-accent/50 transition-colors">
                    <TableCell>
                      <div class="flex items-center gap-2">
                        <div class="size-7 rounded-full flex items-center justify-center text-white text-[10px] font-bold shrink-0" :class="getAvatarColor(tc.employeeName || '')">
                    ...
                    <TableCell class="text-xs tabular-nums">
                      {{ fmtShortDate(extractDateStr(tc.clockIn) || extractDateStr(tc.scheduleDate) || extractDateStr(tc.createdAt)) }}
                    </TableCell>
                    <TableCell>
          ...
          <div v-else class="flex flex-col items-center justify-center py-16 text-center">
            <div class="size-14 rounded-full bg-muted flex items-center justify-center mb-3">
              <Icon name="i-lucide-timer" class="size-7 text-muted-foreground/50" />
            </div><h3 class="text-sm font-bold">
              No Time Cards
            </h3><p class="text-xs text-muted-foreground mt-1">
              No time cards logged for this schedule.
            </p>
          </div>
        </div>
        ...
        <div v-else-if="activeTab === 'djt'">
            ...
            <div class="space-y-6">
              ...
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                  <div class="size-10 rounded-lg bg-gradient-to-br from-orange-500 to-orange-600 flex items-center justify-center shadow-md shadow-orange-500/20">
                    <Icon name="i-lucide-clipboard-list" class="size-5 text-white" />
                  </div>
                  <div>
                    <h2 class="text-base font-bold">
                      Daily Job Ticket
                    </h2><p class="text-xs text-muted-foreground">
                      {{ djt.scheduleTitle || schedule.title }}
                    </p>
                  </div>
                </div>
                <div class="flex items-center gap-2">
                  <Badge v-if="djt.hasCustomerSignature" class="bg-emerald-500/10 text-emerald-600 border-emerald-500/20 text-[10px] font-bold">
                    <Icon name="i-lucide-check-circle" class="size-3 mr-1" />Customer Signed
                  </Badge>
                  <Badge v-else variant="secondary" class="text-[10px] font-bold">
                    <Icon name="i-lucide-alert-circle" class="size-3 mr-1" />Unsigned
                  </Badge>
                </div>
              </div>
              <!-- DJT Stats -->
              ...
              <Card v-if="djt.dailyJobDescription">
                <CardHeader class="pb-3">
                  <CardTitle class="text-sm font-bold flex items-center gap-2">
                    <Icon name="i-lucide-file-text" class="size-4 text-orange-500" />Daily Job Description
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <p class="text-sm text-foreground/80 leading-relaxed whitespace-pre-wrap">
              ...
              <Card v-if="djt.equipmentUsed?.length > 0">
                <CardHeader class="pb-3">
                  <CardTitle class="text-sm font-bold flex items-center gap-2">
                    <Icon name="i-lucide-truck" class="size-4 text-blue-500" />Equipment Used<Badge variant="secondary" class="text-[10px] font-bold ml-auto">
                      {{ djt.equipmentUsed.length }}
                    </Badge>
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <div v-for="(eq, idx) in djt.equipmentUsed" :key="idx" class="flex items-center gap-2 p-2 rounded-md bg-muted/30">
                      <Icon name="i-lucide-wrench" class="size-3.5 text-blue-500 shrink-0" /><span class="text-xs font-medium">{{ eq.name || eq.equipment || eq }}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card v-if="djt.signatures?.length > 0">
                <CardHeader class="pb-3">
                  <CardTitle class="text-sm font-bold flex items-center gap-2">
                    <Icon name="i-lucide-pen-tool" class="size-4 text-violet-500" />Signatures<Badge variant="secondary" class="text-[10px] font-bold ml-auto">
                      {{ djt.signatures.length }}
                    </Badge>
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    <div v-for="(sig, idx) in djt.signatures" :key="idx" class="flex items-center gap-3 p-3 rounded-lg bg-muted/30">
                      <Avatar class="size-9 border">
                        <AvatarImage v-if="sig.employeeAvatar" :src="sig.employeeAvatar" /><AvatarFallback class="text-[10px] font-bold" :class="getAvatarColor(sig.employeeName || '')">
                          {{ getInitials(sig.employeeName || '?') }}
                        </AvatarFallback>
                      </Avatar>
                      <div class="flex-1 min-w-0">
                        <p class="text-xs font-semibold truncate">
                          {{ sig.employeeName || 'Unknown' }}
                        </p><p class="text-[10px] text-muted-foreground">
                          {{ sig.clockOut ? fmtTime(sig.clockOut) : 'Signed' }}
                        </p>
                      </div>
          ...
          <div v-else class="flex flex-col items-center justify-center py-16 text-center">
            <div class="size-14 rounded-full bg-muted flex items-center justify-center mb-3">
              <Icon name="i-lucide-clipboard-list" class="size-7 text-muted-foreground/50" />
            </div><h3 class="text-sm font-bold">
              No Job Ticket
            </h3><p class="text-xs text-muted-foreground mt-1">
              No daily job ticket has been created for this schedule.
            </p>
          </div>
        </div>
        ...
        <div v-else-if="activeTab === 'jha'">
          <template v-if="jha">
            <div class="space-y-6">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                  <div class="size-10 rounded-lg bg-gradient-to-br from-rose-500 to-rose-600 flex items-center justify-center shadow-md shadow-rose-500/20">
                    <Icon name="i-lucide-shield-alert" class="size-5 text-white" />
                  </div>
                  <div>
                    <h2 class="text-base font-bold">
                      Job Hazard Analysis
                    </h2><p class="text-xs text-muted-foreground">
                      {{ jha.scheduleTitle || schedule.title }}
                    </p>
                  </div>
                </div>
                <div class="flex items-center gap-2">
                  <Badge v-if="jha.hasAllAssigneeSigns" class="bg-emerald-500/10 text-emerald-600 border-emerald-500/20 text-[10px] font-bold">
                    <Icon name="i-lucide-check-circle" class="size-3 mr-1" />All Signed
                  </Badge>
                  <Badge v-else variant="secondary" class="text-[10px] font-bold">
                    <Icon name="i-lucide-alert-circle" class="size-3 mr-1" />{{ jha.unsignedAssigneeCount || 0 }} Unsigned
                  </Badge>
                  <Badge v-if="jha.hasClientSign" class="bg-blue-500/10 text-blue-600 border-blue-500/20 text-[10px] font-bold">
                    <Icon name="i-lucide-user-check" class="size-3 mr-1" />Client Signed
                  </Badge>
                </div>
              </div>
              <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                ...
                <div class="rounded-lg bg-muted/30 p-3 ring-1 ring-border/30">
                  <p class="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                    Service
                  </p><p class="text-sm font-bold truncate">
                    {{ jha.service || '—' }}
                  </p>
                </div>
                <div class="rounded-lg bg-muted/30 p-3 ring-1 ring-border/30">
              ...
              <Card v-if="jha.hazards || jha.controls || jha.ppe">
                <CardHeader class="pb-3">
                  <CardTitle class="text-sm font-bold flex items-center gap-2">
                    <Icon name="i-lucide-alert-triangle" class="size-4 text-amber-500" />Hazard Assessment
                  </CardTitle>
                </CardHeader>
                <CardContent class="space-y-4">
                  <div v-if="jha.hazards">
                    <p class="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-1">
                      Hazards
                    </p><p class="text-sm text-foreground/80 leading-relaxed whitespace-pre-wrap">
                      {{ jha.hazards }}
                    </p>
                  </div>
                  <div v-if="jha.controls">
                    <p class="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-1">
                      Controls
                    </p><p class="text-sm text-foreground/80 leading-relaxed whitespace-pre-wrap">
                      {{ jha.controls }}
              ...
              <Card v-if="jha.signatures?.length > 0">
                <CardHeader class="pb-3">
                  <CardTitle class="text-sm font-bold flex items-center gap-2">
                    <Icon name="i-lucide-pen-tool" class="size-4 text-violet-500" />Signatures<Badge variant="secondary" class="text-[10px] font-bold ml-auto">
                      {{ jha.signatures.length }} / {{ jhaAssigneeCount }}
                    </Badge>
                  </CardTitle>
                </CardHeader>
                <CardContent>
          ...
          <div v-else class="flex flex-col items-center justify-center py-16 text-center">
            <div class="size-14 rounded-full bg-muted flex items-center justify-center mb-3">
              <Icon name="i-lucide-shield-alert" class="size-7 text-muted-foreground/50" />
            </div><h3 class="text-sm font-bold">
              No JHA
            </h3><p class="text-xs text-muted-foreground mt-1">
              No Job Hazard Analysis has been created for this schedule.
            </p>
          </div>
        </div>
        ...
        <div v-else-if="activeTab === 'tasks'" class="space-y-4">
          <template v-if="tasks.length > 0">
            <div class="flex items-center gap-3 mb-4">
              ...
              <Badge v-if="todoTasks.length" class="bg-amber-500/10 text-amber-600 border-amber-500/20 text-[10px] font-bold">
                {{ todoTasks.length }} todo
              </Badge>
              <Badge v-if="doneTasks.length" class="bg-emerald-500/10 text-emerald-600 border-emerald-500/20 text-[10px] font-bold">
                {{ doneTasks.length }} done
              </Badge>
              <p class="text-[10px] text-muted-foreground ml-auto">
                Tasks for estimate {{ schedule.estimate }}
              </p>
            </div>
            <div class="space-y-2">
              <div v-for="(task, idx) in tasks" :key="task._id || idx" class="flex items-center gap-3 p-3 rounded-lg border bg-card hover:bg-accent/30 transition-colors group">
                ...
                <div class="flex-1 min-w-0">
                  <p class="text-sm font-medium" :class="(task.status || '').toLowerCase() === 'done' ? 'line-through text-muted-foreground' : ''">
                    {{ task.task || 'Untitled Task' }}
                  </p>
                  <div v-if="task.createdAt" class="text-[10px] text-muted-foreground mt-0.5">
          ...
          <div v-else class="flex flex-col items-center justify-center py-16 text-center">
            <div class="size-14 rounded-full bg-muted flex items-center justify-center mb-3">
              <Icon name="i-lucide-check-square" class="size-7 text-muted-foreground/50" />
            </div><h3 class="text-sm font-bold">
              No Tasks
            </h3><p class="text-xs text-muted-foreground mt-1">
              No tasks found for estimate {{ schedule.estimate || '—' }}.
            </p>
          </div>
        </div>
        ...
        <div v-else-if="activeTab === 'chats'" class="space-y-4">
            ...
            <div class="space-y-3">
              <div v-for="(chat, idx) in sortedChats" :key="chat._id || idx" class="flex gap-3 group">
                <Avatar class="size-8 border shrink-0 mt-0.5">
                  ...
                  <AvatarFallback class="text-[10px] font-bold" :class="getAvatarColor(chat.senderName || chat.user || 'U')">
                    {{ getInitials(chat.senderName || chat.user || 'U') }}
                  </AvatarFallback>
                </Avatar>
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2 mb-0.5">
                    <p class="text-xs font-bold">
                      {{ chat.senderName || chat.user || 'Unknown' }}
                    </p>
          ...
          <div v-else class="flex flex-col items-center justify-center py-16 text-center">
            <div class="size-14 rounded-full bg-muted flex items-center justify-center mb-3">
              <Icon name="i-lucide-message-circle" class="size-7 text-muted-foreground/50" />
            </div><h3 class="text-sm font-bold">
              No Chats
            </h3><p class="text-xs text-muted-foreground mt-1">
              No chat messages for this schedule.
            </p>
          </div>
        </div>
      </div>
