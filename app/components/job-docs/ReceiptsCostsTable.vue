...
const baseItems = computed(() => {
  if (!props.estimateNumber) return allRecords.value
...
const filteredItems = computed(() => {
  ...
  if (activeTab.value === 'invoice') items = items.filter(r => (r.type || '').toLowerCase() === 'invoice')
  else if (activeTab.value === 'receipt') items = items.filter(r => (r.type || '').toLowerCase() === 'receipt')
  else if (activeTab.value === 'approved') items = items.filter(r => (r.approvalStatus || '').toLowerCase() === 'approved')
  else if (activeTab.value === 'not-approved') items = items.filter(r => (r.approvalStatus || '').toLowerCase() === 'not approved')
...
const sortedItems = computed(() => {
  ...
  items.sort((a, b) => {
    ...
    let result = 0
    if (key === 'createdAt' || key === 'date' || key === 'dueDate') result = new Date(av).getTime() - new Date(bv).getTime()
...
function getTabCount(key: string): number {
  ...
  if (key === 'invoice') return all.filter(r => (r.type || '').toLowerCase() === 'invoice').length
  if (key === 'receipt') return all.filter(r => (r.type || '').toLowerCase() === 'receipt').length
  if (key === 'approved') return all.filter(r => (r.approvalStatus || '').toLowerCase() === 'approved').length
  if (key === 'not-approved') return all.filter(r => (r.approvalStatus || '').toLowerCase() === 'not approved').length
  return 0
}
...
<template>
  <div class="flex flex-col h-full">
    ...
    <div v-if="fetchError" class="shrink-0 m-4 rounded-lg border border-destructive/30 bg-destructive/5 p-4 flex items-center gap-3">
      ...
      <div class="flex-1">
        <p class="text-sm font-medium text-destructive">Failed to load records</p>
        ...
        <thead class="sticky top-0 z-10 bg-background border-b">
          <tr>
            ...
            <th class="px-3 py-2 text-left font-medium text-muted-foreground cursor-pointer hover:text-foreground select-none" @click="toggleSort('date')">
              <div class="flex items-center gap-1">Date <Icon v-if="sortKey === 'date'" :name="sortDir === 'asc' ? 'i-lucide-arrow-up' : 'i-lucide-arrow-down'" class="size-3 text-primary" /></div>
            </th>
            ...
            <th v-if="!estimateNumber" class="px-3 py-2 text-left font-medium text-muted-foreground cursor-pointer hover:text-foreground select-none" @click="toggleSort('estimateNumber')">
              <div class="flex items-center gap-1">Estimate <Icon v-if="sortKey === 'estimateNumber'" :name="sortDir === 'asc' ? 'i-lucide-arrow-up' : 'i-lucide-arrow-down'" class="size-3 text-primary" /></div>
            </th>
            <th class="px-3 py-2 text-left font-medium text-muted-foreground cursor-pointer hover:text-foreground select-none" @click="toggleSort('vendor')">
              <div class="flex items-center gap-1">Vendor <Icon v-if="sortKey === 'vendor'" :name="sortDir === 'asc' ? 'i-lucide-arrow-up' : 'i-lucide-arrow-down'" class="size-3 text-primary" /></div>
            </th>
            <th class="px-3 py-2 text-left font-medium text-muted-foreground cursor-pointer hover:text-foreground select-none" @click="toggleSort('amount')">
              <div class="flex items-center gap-1">Amount <Icon v-if="sortKey === 'amount'" :name="sortDir === 'asc' ? 'i-lucide-arrow-up' : 'i-lucide-arrow-down'" class="size-3 text-primary" /></div>
            </th>
            <th class="px-3 py-2 text-left font-medium text-muted-foreground">Approval</th>
            <th class="px-3 py-2 text-left font-medium text-muted-foreground">Payment Status</th>
            <th class="px-3 py-2 text-left font-medium text-muted-foreground">Created By</th>
            <th class="px-3 py-2 text-left font-medium text-muted-foreground">Files</th>
        ...
        <tbody class="divide-y divide-border/50">
          <tr v-if="!isFetched && !fetchError">
            <td :colspan="11" class="h-32 text-center">
              <div class="flex flex-col items-center gap-2 text-muted-foreground"><Icon name="i-lucide-loader-2" class="size-6 animate-spin" /><p class="text-sm">Loading records...</p></div>
            </td>
          </tr>
          ...
          <tr v-else-if="isFetched && sortedItems.length === 0">
            <td :colspan="11" class="py-16">
              <div class="flex flex-col items-center text-center">
                <div class="size-16 rounded-full bg-muted flex items-center justify-center mb-4"><Icon name="i-lucide-wallet" class="size-8 text-muted-foreground/50" /></div>
                <h3 class="text-lg font-bold">No Receipts & Costs</h3>
                <p class="text-sm text-muted-foreground max-w-xs mt-1">{{ search ? 'No records match your search.' : estimateNumber ? 'No records for this estimate.' : 'No receipt/cost records found.' }}</p>
          ...
          <template v-for="(rc, index) in displayedItems" :key="rc.id">
            <tr class="hover:bg-muted/30 transition-colors cursor-pointer" :class="{ 'bg-muted/20': expandedId === rc.id }" @click="toggleExpand(rc.id)">
              ...
              <td class="px-3 py-2.5 whitespace-nowrap tabular-nums font-medium">{{ rc.date || '—' }}</td>
              <td class="px-3 py-2.5 whitespace-nowrap">
                <Badge variant="secondary" class="text-[9px]" :class="{ 'bg-violet-500/10 text-violet-600 border-violet-500/20': (rc.type || '').toLowerCase() === 'invoice', 'bg-teal-500/10 text-teal-600 border-teal-500/20': (rc.type || '').toLowerCase() === 'receipt' }">{{ rc.type || '—' }}</Badge>
              </td>
              <td v-if="!estimateNumber" class="px-3 py-2.5 whitespace-nowrap">
                <Badge v-if="rc.estimateNumber" variant="outline" class="text-[10px] font-bold tabular-nums">{{ rc.estimateNumber }}</Badge>
                <span v-else class="text-muted-foreground">—</span>
              </td>
              <td class="px-3 py-2.5 max-w-[150px]"><p class="truncate font-medium" :title="rc.vendor">{{ rc.vendor || '—' }}</p></td>
              ...
              <td class="px-3 py-2.5 text-center">
                <Badge class="text-[9px] h-4 px-1.5" :class="(rc.approvalStatus || '').toLowerCase() === 'approved' ? 'bg-emerald-500/15 text-emerald-600 border-emerald-500/20 hover:bg-emerald-500/20' : 'bg-red-500/10 text-red-500 border-red-500/20 hover:bg-red-500/15'">{{ rc.approvalStatus || '—' }}</Badge>
              </td>
              <td class="px-3 py-2.5 whitespace-nowrap">
                <Badge v-if="rc.status" variant="secondary" class="text-[9px]" :class="{ 'bg-emerald-500/10 text-emerald-600': rc.status.toLowerCase().includes('paid') }">{{ rc.status }}</Badge>
                <span v-else class="text-muted-foreground">—</span>
              </td>
              <td class="px-3 py-2.5">
                <div v-if="rc.createdByName" class="flex items-center gap-1.5">
                  <Avatar class="size-5 border"><AvatarImage v-if="rc.createdByAvatar" :src="rc.createdByAvatar" /><AvatarFallback class="text-[7px]">{{ getInitials(rc.createdByName) }}</AvatarFallback></Avatar>
              ...
              <td class="px-3 py-2.5">
                <Badge v-if="rc.uploadCount > 0" variant="outline" class="text-[9px] h-4 gap-0.5 tabular-nums"><Icon name="i-lucide-paperclip" class="size-2.5" /> {{ rc.uploadCount }}</Badge>
              </td>
              <td class="px-3 py-2.5">
                ...
                <div class="p-4 space-y-4">
                  <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    ...
                    <div class="rounded-lg border bg-card p-3 space-y-3">
                      <h4 class="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5"><Icon name="i-lucide-info" class="size-3.5 text-blue-500" /> Details</h4>
                      <div class="grid gap-2 text-[11px]">
                        <div class="flex justify-between"><span class="text-muted-foreground">Estimate</span><Badge variant="outline" class="text-[10px] font-bold tabular-nums">{{ rc.estimateNumber || '—' }}</Badge></div>
                        <div class="flex justify-between"><span class="text-muted-foreground">Type</span><span class="font-medium">{{ rc.type }}</span></div>
                        <div class="flex justify-between"><span class="text-muted-foreground">Vendor</span><span class="font-medium">{{ rc.vendor || '—' }}</span></div>
                        <div class="flex justify-between"><span class="text-muted-foreground">Date</span><span class="font-medium">{{ rc.date || '—' }}</span></div>
                        <div v-if="rc.dueDate" class="flex justify-between"><span class="text-muted-foreground">Due Date</span><span class="font-medium">{{ rc.dueDate }}</span></div>
                        <div class="flex justify-between items-center pt-2 border-t mt-1"><span class="font-bold text-muted-foreground uppercase tracking-wider">Amount</span><span class="text-base font-bold text-emerald-600">{{ formatCurrency(rc.amount) }}</span></div>
                      </div>
                      <div v-if="rc.customerName" class="pt-2 border-t">
                        <div class="flex items-center gap-2"><Icon name="i-lucide-building" class="size-3.5 text-muted-foreground" /><div><p class="text-[10px] font-semibold">{{ rc.customerName }}</p><p class="text-[9px] text-muted-foreground">Client</p></div></div>
                      </div>
                    </div>

                    <!-- Status & Payment -->
                    <div class="rounded-lg border bg-card p-3 space-y-3">
                      <h4 class="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5"><Icon name="i-lucide-check-circle-2" class="size-3.5 text-emerald-500" /> Payment & Approval</h4>
                      <div class="space-y-3">
                        <div class="flex items-center gap-3 p-2 rounded-md" :class="(rc.approvalStatus || '').toLowerCase() === 'approved' ? 'bg-emerald-500/5 border border-emerald-500/20' : 'bg-red-500/5 border border-red-500/20'">
                          ...
                          <div><p class="text-[10px] font-semibold">{{ rc.approvalStatus || 'Pending' }}</p><p class="text-[9px] text-muted-foreground">Approval Status</p></div>
                        </div>
                        <div class="flex items-center gap-3 p-2 rounded-md bg-muted/30 border">
                          ...
                          <div><p class="text-[10px] font-semibold">{{ rc.status || 'Unknown' }}</p><p class="text-[9px] text-muted-foreground">Payment Status</p></div>
                        </div>
                        <div v-if="rc.paidByName || rc.paidBy" class="text-[10px]"><span class="text-muted-foreground">Paid By:</span> <span class="font-medium">{{ rc.paidByName || rc.paidBy }}</span></div>
                        <div v-if="rc.paymentDate" class="text-[10px]"><span class="text-muted-foreground">Payment Date:</span> <span class="font-medium">{{ rc.paymentDate }}</span></div>
                      </div>
                    </div>
                    ...
                    <div class="space-y-4">
                      <div v-if="rc.tagCount > 0" class="rounded-lg border bg-card p-3 space-y-2">
                        <h4 class="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5"><Icon name="i-lucide-tag" class="size-3.5 text-orange-500" /> Tagged ({{ rc.tagCount }})</h4>
                        <div class="flex flex-wrap gap-1.5">
                          <div v-for="(tag, ti) in rc.tags" :key="ti" class="flex items-center gap-1.5 px-2 py-1 rounded-full bg-muted/50 border text-[10px]">
                            <Avatar class="size-4 border"><AvatarImage v-if="tag.avatar" :src="tag.avatar" /><AvatarFallback class="text-[6px]">{{ getInitials(tag.name) }}</AvatarFallback></Avatar>
                            <span class="font-medium">{{ tag.name }}</span>
                          </div>
                        </div>
                      </div>
                      <div v-if="rc.uploadCount > 0" class="rounded-lg border bg-card p-3 space-y-2">
                        <h4 class="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5"><Icon name="i-lucide-paperclip" class="size-3.5 text-indigo-500" /> Attachments ({{ rc.uploadCount }})</h4>
                      ...
                      <div v-if="rc.remarks" class="rounded-lg border bg-card p-3 space-y-2">
                        <h4 class="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5"><Icon name="i-lucide-message-square" class="size-3.5 text-gray-500" /> Remarks</h4>
                        <p class="text-[10px] text-foreground/80 whitespace-pre-line">{{ rc.remarks }}</p>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <tr v-if="hasMore" ref="scrollSentinel">
            <td :colspan="11" class="py-4 text-center">
              <div class="flex items-center justify-center gap-2 text-muted-foreground"><Icon name="i-lucide-loader-2" class="size-4 animate-spin" /><span class="text-xs">Loading more...</span></div>
            </td>
          </tr>
        </tbody>
      </table>
